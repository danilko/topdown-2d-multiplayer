        ...
        if (agent != null && IsInstanceValid(agent))
        {
            ...
            if (inventory != null && IsInstanceValid(inventory))
            {
                ...
                if (inventory.GetItem(inventoryIndex) != null && !inventory.IsItemIndexInUsed(inventoryIndex))
                {
                    ...
                    if (itemResource.ItemID == "SYC-1200" && agent.IsMoreRemoteWeaponUnitsAllowed())
                    {
                        if (agent.IsMoreRemoteWeaponUnitsAllowed())
                        {
                            agent.AddRemoteWeapon(itemResource.ReferencePackedScene);
                        }
                        isItemUsed = true;
                    }
                    if (itemResource.ItemID == "SYC-10000" && agent.GetEngineType() != Agent.EngineType.NuclearReactor)
                    {
